...
private:
    static void log(char const*) { /* fmt::print("log \"{}\"\n", msg);*/
    }
    static void log_error(char const* msg) { fmt::print("jlink error: \"{}\"\n", msg); }

    void connect(std::string const& device, std::uint32_t speed) {
        {
            char const* ret = JLINK_OpenEx(log, log_error);
            if(ret != nullptr) {
                throw std::runtime_error{fmt::format("JLINK_OpenEx failed: {}", ret)};
            }
        }

        {
            ...
            if(ret != 0) {
                throw std::runtime_error{fmt::format("JLINK_TIF_Select failed: {}", ret)};
            }
        }

        JLINK_SetSpeed(speed);
        preConnectDisableDialogs();
        execCommand(fmt::format("device = {}", device));
        {
            char const ret = JLINK_IsConnected();
            if(ret == 0) {
                int const ret2 = JLINK_Connect();
                if(ret2 != 0) {
                    throw std::runtime_error{fmt::format("JLINK_Connect failed: {}", ret2)};
                }
            }
        }
        ...
        while(trys != 0) {
            ...
            if(ret == 0) {
                ...
                throw std::runtime_error{fmt::format("JLINK_IsConnected failed: {:d}", ret)};
            }
    ...
    void execCommand(std::string const& cmd) {
        {
            ...
            if(ret != 0) {
                ...
                throw std::runtime_error{
                  fmt::format("JLINK_ExecCommand(\"{}\") failed: {}", cmd, error_msg)};
            }
        }
    }
    ...
    void checkError() {
        ...
        if(ret != 0) {
            throw std::runtime_error{fmt::format("JLINK_HasError: {}", ret)};
        }
    }
...
public:
      ...
      std::uint16_t      port = 19020) {
        {
            ...
            if(ret != 0) {
                throw std::runtime_error{
                  fmt::format("JLINK_SelectIP({}, {}) failed: {}", ip, port, ret)};
            }
        }
    ...
    explicit JLink(std::string const& device, std::uint32_t speed) {
        {
            ...
            if(ret != 0) {
                throw std::runtime_error{fmt::format("JLINK_SelectUSB failed: {}", ret)};
            }
        }
        connect(device, speed);
        checkError();
    ...
    void startRtt(std::uint32_t buffers, std::uint32_t configBlockAddress = 0) {
        auto config = [](std::uint32_t address) {
            ...
            if(ret < 0) {
                throw std::runtime_error{fmt::format("JLINK_RTTERMINAL_Control failed: {}", ret)};
            }
        };
        auto connect = [&]() {
            std::size_t trys{100};
    ...
    std::span<std::byte> rttRead(std::uint32_t bufferNumber, std::span<std::byte> buffer) {
        int const ret = JLINK_RTTERMINAL_Read(
          ...
          buffer.size());
        if(ret < 0) {
            throw std::runtime_error{fmt::format("JLINK_RTTERMINAL_Read failed: {}", ret)};
        }
    ...
    void checkConnected() {
        ...
        char const ret = JLINK_IsConnected();
        if(ret == 0) {
            throw std::runtime_error{fmt::format("JLINK_Connected: {}", ret)};
        }
    }
    ...
    void resetTarget() {
        ...
        if(ret < 0) {
            throw std::runtime_error{fmt::format("JLINK_Reset: {}", ret)};
        }
    }
    ...
    void flash(std::string const& hexFile) {
        ...
        if(ret < 0) {
            throw std::runtime_error{fmt::format("JLINK_DownloadFile: {}", ret)};
        }
    }
